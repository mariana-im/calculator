...
def main():   
      def operaciones():
        ...
        while True:
          try:
            opcion=int(input(f"Ingrese el número de acción que desea realizar, {user}: "))
            break
          except ValueError:
            print("La opción que ingreso no es valida")
            continue

        print(opciones_1[int(opcion)-1])
        ...
        resultado=(funciones[opcion-1]())
        return(resultado)
#función que almacena operaciones
      def grafica():
        import numpy as np
        ...
        #cada segundo recorrera 5 grados
        ...
        plt.grid(True)
        plt.show()
#función que almacena operaciones para grafica

      num=0
      acciones_1=["operaciones","graficar"]
